...
/// 处理模态转场动画
/// V2：移除 nav.delegate 逻辑，如果为 nav，则将手势加在 topVC 上（见 XgModalInteractor）
...
class XgModalInteractor: UIPercentDrivenInteractiveTransition {
    ...
    init(viewController: UIViewController) {
        ...
        panGR.edges = .left
        if let nav = viewController as? UINavigationController, let topVC = nav.topViewController {
            topVC.view.addGestureRecognizer(panGR)
        } else {
